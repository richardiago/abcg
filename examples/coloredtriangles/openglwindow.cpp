...
void OpenGLWindow::paintUI() {
  ...
  {
    auto widgetSize{ImVec2(250, 170)};
    ...
    ImGui::Checkbox("Flat color", &flatColor);
    ...
    ImGui::SliderInt("Delay", &m_delay, 0, 500, "%d ms");
    ImGui::PopItemWidth();
...
void OpenGLWindow::setupModel() {
  ...
  if(randomColor){
      ...
      std::uniform_real_distribution<float> rd(0.00f, 1.00f);
      m_vertexColors = { glm::vec4(rd(m_randomEngine), rd(m_randomEngine), rd(m_randomEngine), rd(m_randomEngine)),
                      glm::vec4(rd(m_randomEngine), rd(m_randomEngine), rd(m_randomEngine), rd(m_randomEngine)),
                      glm::vec4(rd(m_randomEngine), rd(m_randomEngine), rd(m_randomEngine), rd(m_randomEngine)) };
     // If Flat colors
     if(flatColor){

         m_vertexColors[1] = m_vertexColors[0];
         m_vertexColors[2] = m_vertexColors[0];
     }
  }
  // If not Random colors
  else {
      // If not Flat colors
      if(flatColor){
         m_vertexColors[1] = m_vertexColors[0];
